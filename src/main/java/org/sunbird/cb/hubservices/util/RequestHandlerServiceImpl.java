...
public class RequestHandlerServiceImpl {
    private final Logger log = LoggerFactory.getLogger(RequestHandlerServiceImpl.class);
    ...
    public Map<String, Object> fetchResultUsingPost(String uri, Object request, Map<String, String> headersValues) {
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                String str = String.format("Response: %s%n", mapper.writeValueAsString(response));
                log.debug(str);
            }
            ...
            try {
                ...
                log.error("Error parsing response: " + e1.getMessage(), e1);
            }
            log.error("Error received: " + hce.getResponseBodyAsString(), hce);
        } catch (JsonProcessingException e) {
            log.error(String.valueOf(e));
            try {
                log.warn(String.format("Error Response: %s", mapper.writeValueAsString(response)));
            } catch (Exception e1) {
                log.error("Error writing response: " + e1.getMessage(), e1);
            }
        }
        return response;
    }
